...
export function ProductCard({ product }: { product: Product }) {
  return <Link href={`/products/${product.slug}`} className="product-card group">
    ...
    <div className="flex flex-1 flex-col p-5">
      ...
      <div className="mt-5 flex items-end justify-between border-t border-line pt-4"><div><p className="text-xs text-ink-soft">Suggested retail</p><p className="font-semibold text-ink">{formatCurrency(product.msrpCents)}</p></div><span className="text-xs font-semibold text-ember-dark">Dealer pricing protected</span></div>
    </div>
  </Link>;
}
